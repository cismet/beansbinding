...
public abstract class Binding<SS, SV, TS, TV> {
    ...
    public static final class SyncFailure {
        private static final org.apache.log4j.Logger log = org.apache.log4j.Logger.getLogger(SyncFailure.class);
        ...
        private static SyncFailure validationFailure(Validator.Result result) {
         ...
         private static SyncFailure validationWarning(Validator.Result result) {
            
            SyncFailure ret= new SyncFailure(result);
            ret.type=SyncFailureType.VALIDATION_WARNING;
            return ret;
        }
        ...
        public Validator.Result getValidationResult() {
            if (type == SyncFailureType.VALIDATION_FAILED || type== SyncFailureType.VALIDATION_WARNING) {
    ...
    public static final class ValueResult<V> {
        ...
        private ValueResult(V value,SyncFailure failure) {
            this.value = value;
    ...
    public final ValueResult<SV> getTargetValueForSource() {
        ...
        if (rawValue == null) {
            if (validator != null) {
                ...
                if (vr != null) {
                    if (vr.getType()!=null&&vr.getType().equals(Validator.Result.ERROR)) {
                        return new ValueResult<SV>(SyncFailure.validationFailure(vr));
                    } else {
                        return new ValueResult<SV>((SV) value, SyncFailure.validationWarning(vr));
    ...
    protected final void notifySyncFailed(SyncFailure failure) {
        //log.fatal("notifySyncFailed",new CST());
        if (listeners == null) {
            return;
        }
    ...
    /**
     * Notifies all registered {@code BindingListeners} of a warning to
     * sync ({@code refresh} or {@code save}), by calling
     * {@code syncFailed} on each one.
     *
     * @param failure the reason that the sync warning
     */
    protected final void notifySyncWarning(SyncFailure failure) {
        if (listeners == null) {
            return;
        }

        for (BindingListener listener : listeners) {
            listener.syncWarning(this, failure);
        }
    }

    private final SyncFailure notifyAndReturn(SyncFailure failure) {
        //log.fatal("notifyAndReturn");
        if (failure == null) {
            notifySynced();
        } else {
            if (failure.getType() == SyncFailureType.VALIDATION_WARNING) {
                //Warning
                notifySynced();
                notifySyncWarning(failure);
            } else {
    ...
    protected final SyncFailure saveUnmanaged() {
        //log.fatal("saveUnmanaged",new CST());
    ...
    public static class CST extends Throwable{
