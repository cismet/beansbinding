...
public class AutoBinding<SS, SV, TS, TV> extends Binding<SS, SV, TS, TV> {
    ...
    private final void tryRefreshThenSave() {
        ...
        if (refreshFailure == null) {
            ...
            if (saveFailure == null) {
                ...
                if (saveFailure.getType() == SyncFailureType.VALIDATION_WARNING) {
                    notifySyncWarning(refreshFailure);
                } else {
    ...
    private final void trySaveThenRefresh() {
        ...
        } else if (saveFailure.getType() == SyncFailureType.VALIDATION_WARNING) {
            notifySyncWarning(saveFailure);
        } else {
            SyncFailure refreshFailure = refresh();
            if (refreshFailure == null) {
                notifySynced();
