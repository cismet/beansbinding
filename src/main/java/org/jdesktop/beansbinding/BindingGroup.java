...
public class BindingGroup {
    ...
    public void bind() {
        ...
        boolean debug = false;
        if (System.getProperty("cismet.beansdebugging") != null && System.getProperty("cismet.beansdebugging").equals("true")) {
            debug = true;
            log.debug("BindingGroup.bind()");
        }
        for (Binding binding : toBind) {
            if (debug) {
                try {
                    ...
                    log.error("Error during bind. Binding:"+binding.getName()+
                            "\n" +binding.toString(),e);
                }
                }
            else  {
    ...
    private class Handler implements BindingListener {
        ...
        public void syncWarning(Binding binding, SyncFailure failure) {
            ...
            for (BindingListener listener : listeners) {
                listener.syncWarning(binding, failure);
            }
        }
